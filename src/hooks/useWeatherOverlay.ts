...
// Provides functions to draw gradient overlay, particles, and lightning
 ...
 * particles, and lightning onto a canvas rendering context.
...
export function useWeatherOverlay({
  ...
  const renderOverlay = useCallback((
    ...
    if (fieldChanged || cameraChanged || !offscreenRef.current) {
    ...
    // ── DM-only analytical overlays (isobars, fronts) ──
    if (isDMView) {
      ctx.save();
  ...
  }, [field, config, gridWidth, gridHeight, isDMView]);
